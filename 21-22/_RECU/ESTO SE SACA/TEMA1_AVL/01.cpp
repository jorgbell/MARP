...
/*@ <answer>
 ...
 Un AVL es un arbol que tiene que comprobar que:
    ...
    -el valor almacenado en la ra�z tiene que ser estrictamente mayor que los de la izq y menor que los de la der
Hacer un m�todo para comprobar cada una de esas cosas
 @ </answer> */
...
bool esABB(bintree<int> tree, int minValue, int maxValue) {
    //m�todo recursivo que comprueba si cada subarbol es abb comparando los valores minimos y maximos que deberian llegar los hijos
    ...
    if (tree.root() < minValue || tree.root() > maxValue)
        return false; //en caso de ocurrir esto ser� un arbol erroneo
    //llamada recursiva
...
bool resolverArbol(bintree<int> tree) {
    ...
    //CASO BASE:
    if (tree.empty())
        return true; //uno vac�o siempre ser� AVL
...
void resuelveCaso5() {
    ...
    bintree<int> tree = leerArbol(-1); //caracter con el que indicamos qu� es un vac�o
